    ...
    for j in range(1, len(dta["lines"])+1):
        ...
        for i in pnter:
            if i in allvars and pnter[i] in allvars:
                d.line((xval[i], yval[i],max(xval[i], xval[pnter[i]]) + 20,yval[i]), fill=(255,0,0), width=2)
                d.line((max(xval[i], xval[pnter[i]]) + 20, yval[i],max(xval[i], xval[pnter[i]]) + 20,yval[pnter[i]]), fill=(255,0,0), width=2)
                d.line((xval[pnter[i]],yval[pnter[i]],max(xval[i], xval[pnter[i]]) + 20,yval[pnter[i]]), fill=(255,0,0), width=2)
